...
public class FaceLiveness {
    ...
    private boolean onLivenessCheck(int width, int height, int type) {
        ...
        //  克隆基本数据，保证一致
        int[] cloneRgbArray = mRgbArray.clone();

        long startTime = System.currentTimeMillis();
        FaceInfo[] faceInfos = null;
        if (cloneRgbArray != null) {
            faceInfos = FaceSDKManager.getInstance().getFaceDetector().trackMaxFace(cloneRgbArray, width, height);
        }
        LivenessModel livenessModel = new LivenessModel();
        livenessModel.setRgbDetectDuration(System.currentTimeMillis() - startTime);
        livenessModel.getImageFrame().setArgb(cloneRgbArray);
        ...
        if (faceInfos != null && faceInfos.length > 0) {
            ...
            if (faceInfos[0].mConf < GlobalSet.getDetectConfValue() || Utils.landmarksSmallZero(faceInfo.landmarks)) {
                return isLiveness;
            }
            // 返回检测到人脸的事件
        ...
        future2 = es2.submit(new Runnable() {
            ...
            public void run() {
                ...
                if ((livenessModel.getLiveType() & MASK_DEPTH) == MASK_DEPTH) {
                    long startTime = System.currentTimeMillis();
                    // 因奥比中光镜头深度的图片分辨率不一样，所以要采用特殊的分辨率传入
                ...
                if (livenessCallBack != null) {
                    ...
                    switch (currentTaskType) {
                        ...
                        case TASK_TYPE_REGIST:
                            if (!TextUtils.isEmpty(registNickName)) {
        ...
        if ((GlobalSet.getLiveStatusValue() == GlobalSet.LIVE_STATUS.NO)
            ...
            if (length == 128) {
                ...
                if (FaceApi.getInstance().featureAdd(feature)) {
                    returnRegistResult(0, livenessModel, cropBitmap, feature);
                }
                registNickName = null;
                return;
            }
     ...
     * @param livenessModel
     */
    private void returnRegistResult(int code, LivenessModel livenessModel, Bitmap cropBitmap, Feature feature) {
        synchronized (mLock) {
            for (IFaceRegistCalllBack faceRegistCalllBack : registCalllBacks) {
                try {
                    faceRegistCalllBack.onRegistCallBack(code, livenessModel, cropBitmap, feature);
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }

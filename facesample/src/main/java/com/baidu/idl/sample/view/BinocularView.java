...
public class BinocularView extends RelativeLayout implements View.OnClickListener, ILivenessCallBack {
    ...
    public BinocularView(Context context, int count) {
        this(context, null, count);
    }

    public BinocularView(Context context, AttributeSet attrs, int count) {
        this(context, attrs, 0, count);
    }

    public BinocularView(Context context, AttributeSet attrs, int defStyleAttr, int count) {
        super(context, attrs, defStyleAttr);
        initview(context, count);
        mLayoutOne.initFaceFrame(context);
    }
    ...
    private void initview(Context context, int count) {
        ...
        mLayoutOne = new BaseCameraView(context);
        ...
        if (count == 1) {
            mBtnGoLeft.setVisibility(GONE);
            mBtnGoRight.setVisibility(GONE);
        } else {
        ...
        mirror = GlobalSet.getMirrorStatus();
    ...
    private void dealRgb(byte[] data) {
        if (rgbData == null) {
            ...
            FaceSDKManager.getInstance().getFaceDetector().yuvToARGB(data, PREFER_WIDTH, PERFER_HEIGH, argb, 0, mirror);
            rgbData = argb;
            // 用于显示送检时的图片，建议注释掉
//            post(new Runnable() {
//                @Override
...
//            });
    ...
    private void dealIr(byte[] data) {
        if (irData == null) {
            ...
            FaceSDKManager.getInstance().getFaceDetector().yuvToARGB(data, PREFER_WIDTH, PERFER_HEIGH, niRargb, 0, mirror);

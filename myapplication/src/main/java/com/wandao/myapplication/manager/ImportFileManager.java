package com.wandao.myapplication.manager;
...
//                }
                ...
                logBuilder.append("ID\t图片名\t成功/失败\t失败原因\n");
                ...
                for (int i = 0; i < lineNums; i++) {
                    ...
                    String userName = lineTxt[2]; // 用户姓名
                    String userID = lineTxt[0];       //用户ID
                    String doorNumber = lineTxt[3];       //用户箱门
                    String department = lineTxt[4];       //部门
                    boolean success = false;

                    // 判断姓名是否有效
                    String nameResult = FaceApi.getInstance().isValidName(userID);
                    if (!"0".equals(nameResult)) {
                        // 保存失败信息
                        logBuilder.append(userID + "\t" + picFile + "\t"
                                + "失败" + "\t" + nameResult + "\n");
                        mFinishCount++;
                        mFailCount++;
                        // 更新进度
                    ...
                    List<Feature> listFeatures = DBManager.getInstance().queryFeatureByName(userID);
                    if (listFeatures != null && listFeatures.size() > 0) {
                        String msg = "与之前图片对应的ID相同";
                        // 保存失败信息
                        logBuilder.append(userID + "\t" + picFile + "\t"
                                + "失败" + "\t" + msg + "\n");
                        mFinishCount++;
                    ...
                    if (picPath.exists()) {
                        ...
                        if (argbImg.width * argbImg.height <= 1000 * 1000) {
                            ...
                            if (ret == -1) {
                                ...
                                feature.setUserName(userID);
                                feature.setCropImageName(cropImgName);


                                User user = new User(Long.parseLong(userID), userName, "", "", department, false, 0, Integer.parseInt(doorNumber));
                                // 保存数据库
                                if (DbUtils.insertUser(user) && FaceApi.getInstance().featureAdd(feature)) {
                                    success = true;
                                    // 保存图片到新目录中
